// GetBalance -> 自分の保持しているCoinの情報を取得するAPI
// Ticker -> 自分の保持しているBTCの情報を取得するAPI
...
// api-headerにmethod/endpoint/bodyを渡す関数 -> 返値にmapで[string]
// *mapとは、[key]valueを組み合わせたデータ構造
func (api APIClient) header(method, endpoint string, body []byte) map[string]string {
	timestamp := strconv.FormatInt(time.Now().Unix(), 10) // timestampを現在日時 -> FormatIntで10進数にする
...
func (api *APIClient) doRequest(method, urlPath string, query map[string]string, data []byte) (body []byte, err error) {
	...
	// mapを返してheaderの中身を[key]valueに格納していく
...
// symbol->BTC_JPY, ch->RealTimeDataを取得
			...
			case map[string]interface{}:
				for key, binary := range v {
					if key == "message" {
						...
						// ålog.Println("marshaTic:", marshaTic)
						if err != nil {
							continue OUTER
						}
						var ticker Ticker
